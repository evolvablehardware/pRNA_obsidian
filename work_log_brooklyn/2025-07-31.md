...
* Basic breakdown of USB protocol:
	...
	* USB Devices can be made up of a bunch of different kinds of endpoints including:
		...
		* Control
	* There is a very (nicely abridged) good writeup on how the USB protocol is setup [here](https://www.beyondlogic.org/usbnutshell/usb1.shtml)
...
* Examples of people doing similar things to what we want to be doing:
	* 
...
* I tried to make my examples agnostic to the two boards by just using the predefined names, **I THINK WE SHOULD TRY HARD TO MAINTAIN THIS STANDARD**, however, there are some inherent hardware differences:
	* Mainly, some of the Pico's IO pins (especially the ADCs) are matched to different pins on the FPGA
	* Further, the pico1 has a single RGB LED that is shared between the FPGA and the pico, wheras the pico2 has separate LEDs, Diagnostic blinking should probably reserve specific LED colors for FGPA vs Pico so that the code can be portable across both boards.
* CMake -DPICO_BOARD=(board name) configures which definitions are used at compiletime
* The examples that use the FPGA **HARD CODE THE  .pcf FILE IN CMakeLists.txt** we should try to configure this to work with the PICO_BOARD define.
...
[[2025-07-30|prev]] [[Notes While In Hawaii|next]]
